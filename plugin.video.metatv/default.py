...
def displaypage2(url):   
   d = dailymotion.Dailymotion()
   video=d.get(url)
   addLink(video["title"], video["id"], 'video', video["thumbnail_720_url"])  
   xbmcplugin.endOfDirectory(addon_handle,succeeded=True,updateListing=False,cacheToDisc=True)   
...
if mode is '':
    d = dailymotion.Dailymotion()
    video = d.get("https://api.dailymotion.com/video/x155tg2?fields=id,thumbnail_720_url,title,url")
    addLink(video["title"], video["id"], 'video', video["thumbnail_720_url"])  
    xbmcplugin.endOfDirectory(addon_handle,succeeded=True,updateListing=False,cacheToDisc=True) 
else:    
  if mode == 'video':
